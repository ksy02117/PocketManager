...
public class EventWeatherReceiver {
    private static EventWeatherReceiver instance = null;
    ...
    private EventWeatherReceiver(Context context) {
        ctx = context;
        requestQueue = Volley.newRequestQueue(context.getApplicationContext());
    }


    public static synchronized EventWeatherReceiver getInstance(Context context) {
        if (instance == null)
            instance = new EventWeatherReceiver(context);
        return instance;
    }

    //this is so you don't need to pass context each time
    public static synchronized EventWeatherReceiver getInstance() {
        if (null == instance) {
            throw new IllegalStateException(EventWeatherReceiver.class.getSimpleName() +
                    " is not initialized, call getInstance(...) first");
        }
        return instance;
    }

    public void getEventWeather(Double latitude, Double longitude, final APIListener<LinkedList<WeatherData>> listener) {
